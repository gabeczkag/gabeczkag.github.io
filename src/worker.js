  ...
  async fetch(req, env) {
    try {
      ...
      if (url.pathname === "/callback") {
        ...
        let access;
        try {
          access = await exchange(code, env);
          ...
          return new Response("Krok: callback\nBlad: " + e.message, { status: 502 });
        }
      }
...
async function exchange(code, env) {
  if (typeof env.GITHUB_CLIENT_SECRET === "undefined") {
  ...
  const tokRes = await fetch("https://github.com/login/oauth/access_token", {
    ...
    body: "client_id=" + encodeURIComponent(CLIENT_ID) +
      "&client_secret=" + encodeURIComponent(env.GITHUB_CLIENT_SECRET) +
